        ...
        <div className="container mt-4 pb-5 text-center">
            ...
            <div className="row g-3 mt-3">
                <div className="col-12 col-md-6">
                    <div className="card bg-white rounded-4 border shadow-sm p-3 h-100 d-flex flex-column">
                        <h5 className="fw-bold mb-2"><i className="fas fa-gem text-info me-2"></i> Buy Gems</h5>
                        <p className="small text-muted mb-3">Purchase Gems securely using Telegram Stars.</p>
                        
                        <div className="row g-2 mt-auto">
                            {starPackages.map(pkg => (
                                <div className="col-6" key={`star_${pkg.id}`}>
                                    <button className="btn btn-outline-info fw-bold w-100 py-3 shadow-sm d-flex flex-column align-items-center justify-content-center h-100" onClick={() => handleBuyGems(pkg.stars)} style={{ transition: 'all 0.2s' }}>
                                        <span className="fs-5 text-dark mb-1 d-flex align-items-center gap-1">{pkg.gems} <i className="fas fa-gem text-info"></i></span>
                                        <span className="badge bg-info text-white shadow-sm">{pkg.stars} ⭐️</span>
                                    </button>
                                </div>
                            ))}
                        </div>
                ...
                <div className="col-12 col-md-6">
                    <div className="card bg-white rounded-4 border shadow-sm p-3 h-100 d-flex flex-column">
                        <h5 className="fw-bold mb-2"><i className="fas fa-exchange-alt text-warning me-2"></i> Exchange Gems</h5>
                        <p className="small text-muted mb-3">Convert your Gems into Credits instantly!</p>
                        
                        <div className="row g-2 mt-auto">
                            {gemPackages.map(pkg => (
                                <div className="col-6" key={`gem_${pkg.id}`}>
                                    <button className="btn btn-warning fw-bold w-100 py-3 shadow-sm text-dark d-flex flex-column align-items-center justify-content-center h-100" disabled={!user || user.gems < pkg.gems} onClick={() => handleExchange(pkg.id, pkg.gems)} style={{ transition: 'all 0.2s' }}>
                                        <span className="fs-5 mb-1 d-flex align-items-center gap-1">{pkg.credits} <i className="fas fa-coins text-dark opacity-75"></i></span>
                                        <span className="badge bg-dark text-white opacity-75 shadow-sm">{pkg.gems} <i className="fas fa-gem text-info"></i></span>
                                    </button>
                                </div>
                            ))}
                            {gemPackages.length === 0 && <div className="col-12"><span className="text-muted small">No packages available.</span></div>}
                        </div>
                    </div>
                </div>
            </div>
...
const LobbyView = ({ user, rooms, setModal, socket, systemConfig }) => {
    ...
    const maxRooms = systemConfig?.maxRooms || 1250;
    ...
    const isRoomLimitReached = totalRooms >= maxRooms;
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                {!isRoomLimitReached ? (
                    ...
                    <div className="text-muted small fw-bold text-end">
                        <i className="fas fa-ban text-danger"></i> Capacity Reached.
                    </div>
                )}
            </div>
