...
const LeaderboardView = ({ socket, tgId, setModal, systemConfig }) => {
    ...
    const renderList = (list, scoreLabel, isPrevInviters = false) => {
        if (!list || list.length === 0) {
            return (
                <div className="text-center p-4 bg-white rounded-4 shadow-sm border mt-3">
                    ...
                    <div className="p-2 border border-primary border-2 rounded bg-light mb-2 shadow-sm d-flex align-items-center justify-content-between">
                        <div className="d-flex align-items-center">
                            <div className="fw-bold text-primary me-3" style={{width: '20px'}}>1</div>
                            <i className="fas fa-user-circle fs-3 text-primary me-2"></i>
                            <span className="fw-bold text-primary">You</span>
                        </div>
                        <span className="badge bg-primary rounded-pill px-3 py-2">0 <small>{scoreLabel}</small></span>
                    </div>
                </div>
            );
        }
        ...
        let myEntry = list.find(u => u.is_me);
        
        // Fix: Force calculation and extraction of user's row to pin at bottom natively if they fell within the top 50 array query
        // and weren't explicitely added to the array by the backend as an 'is_me' item beyond rank 50.
        if (!myEntry) {
                ...
                {top5.map((u, i) => {
                    ...
                    return (
                        <div key={u.tg_id + rank} className={`p-2 border rounded-4 bg-white shadow-sm d-flex align-items-center justify-content-between ${isMe ? 'border-primary border-2' : ''}`}>
                            ...
                            <span className={`badge ${badgeClass} rounded-pill px-3 py-2 shadow-sm`}>{u.score} <small>{scoreLabel}</small></span>
                        </div>
                    );
                })}
        ...
        <div className="container py-2 pb-5 mb-5" style={{maxWidth: '600px'}}>
            ...
            <ul className="nav nav-pills nav-fill mb-4 bg-white rounded-pill shadow-sm p-1 border">
                <li className="nav-item">
                    <button className={`nav-link rounded-pill fw-bold ${activeTab === 'current' ? 'active shadow-sm' : 'text-dark'}`} onClick={() => setActiveTab('current')}>This Week</button>
                </li>
                <li className="nav-item">
                    <button className={`nav-link rounded-pill fw-bold ${activeTab === 'previous' ? 'active shadow-sm' : 'text-dark'}`} onClick={() => setActiveTab('previous')}>Last Week</button>
                </li>
            </ul>

            <div className="row">
                <div className="col-12 mb-4">
                    <div className="d-flex align-items-center bg-white p-3 rounded-4 shadow-sm border-bottom border-primary border-3">
                        <h5 className="fw-bold text-dark mb-0"><i className="fas fa-user-friends text-primary me-2"></i> Top Inviters</h5>
                    </div>
                    {activeTab === 'current' ? renderList(leaderboard.inviters, 'Invites', false) : renderList(leaderboard.prevInviters, 'Invites', true)}
                </div>
                
                <div className="col-12 mb-4">
                    <div className="d-flex align-items-center bg-white p-3 rounded-4 shadow-sm border-bottom border-success border-3">
                        <h5 className="fw-bold text-dark mb-0"><i className="fas fa-check-circle text-success me-2"></i> Top Guessers</h5>
                    </div>
                    {activeTab === 'current' ? renderList(leaderboard.guessers, 'Guesses', false) : renderList(leaderboard.prevGuessers, 'Guesses', false)}
                </div>
            </div>
        </div>
    );
